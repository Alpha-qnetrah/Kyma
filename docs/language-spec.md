...
Parameters always have types and calls always use parentheses. Return annotations are optional for safely inferred functions and mandatory contracts when present. `if` requires a parenthesized condition and braced branches. `while` and `loop (init; condition; increment)` are statements. `break` and `continue` may name a loop label. `match` currently accepts literal and `_` arms; arms use `=>` and terminate with `;`. `if` is expression-capable; loops are not.
