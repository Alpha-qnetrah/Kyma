...
Implemented: lexer, comments and literals; expressions; mandatory semicolon statements; lexical blocks; `let`/`set`, inference, `num`, `any`, nullable type syntax; functions and closures; if/else expressions; while and C-style `loop`; break/continue with loop labels; literal/wildcard match; closed object literals and arrays; classes, instances, init, self, super, single inheritance, method override syntax, static method lookup, and abstract-class instantiation checks; try/catch and error propagation; interfaces are catalogued as compile-time declarations; CLI, REPL, standard-library adapters, VS Code support, tests, and CTest.

The VM, module loader, full access-control checker, structural interface conformance, traits, generic instantiation, maps, and richer patterns are intentionally reserved for the next milestones. Their syntax and AST seams are documented rather than faked by a transpiler.
