...
std::string renderJsonDiagnostics(const std::vector<Diagnostic> &diagnostics,
  ...
  output << "{\"version\":1,\"diagnostics\":[";
  for (std::size_t index = 0; index < diagnostics.size(); ++index) {
    if (index)
      output << ',';
    const auto &diagnostic = diagnostics[index];
    const auto *source = sources.find(diagnostic.location.source);
    output << "{\"code\":\"" << escapeJson(diagnostic.code) << "\",\"severity\":\""
           ...
           << ",\"column\":" << diagnostic.location.endColumn << "}},\"labels\":[";
    ...
    output << "],\"help\":\"" << escapeJson(diagnostic.help) << "\",\"callFrames\":[";
