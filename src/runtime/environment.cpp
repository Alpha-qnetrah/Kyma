...
void Environment::define(const std::string &name, Value value, bool mutableBinding) {
  if (values.contains(name))
    throw KymaError({"binding '" + name + "' is already declared in this scope", {1, 1}, false});
...
Cell &Environment::get(const std::string &name) {
  ...
  throw KymaError({"undefined name '" + name + "'", {1, 1}, false});
}
void Environment::assign(const std::string &name, Value value) {
  auto found = values.find(name);
  if (found != values.end()) {
    if (!found->second.mutableBinding)
      throw KymaError({"cannot assign to immutable binding '" + name + "'", {1, 1}, false});
