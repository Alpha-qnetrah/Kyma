...
const languageSelector = { language: 'kyma', scheme: 'file' };
...
const wordCompletions = [
  ...
  ['import', vscode.CompletionItemKind.Keyword, 'Namespace import', 'import "${1:./module.ky}" as ${2:module};'],
  ...
  ['try', vscode.CompletionItemKind.Keyword, 'Handled runtime failure', 'try {\n\t$1\n} catch (${2:error}) {\n\t$0\n}'],
  ...
  ...['int', 'float', 'num', 'str', 'char', 'bool', 'void', 'any'].map(word =>
    [word, vscode.CompletionItemKind.TypeParameter, `Kyma ${word} type`]),
  ...['print', 'typeOf', 'len', 'push', 'pop', 'keys', 'readFile', 'writeFile', 'processRun',
    ...
    .map(word => [word, vscode.CompletionItemKind.Function, 'Kyma standard-library function']),
  ['fs', vscode.CompletionItemKind.Module, 'Kyma filesystem namespace']
];
...
function executable(document) {
  const configured = vscode.workspace.getConfiguration('kyma').get('executable', '');
  if (configured) return configured;
  const folder = document ? vscode.workspace.getWorkspaceFolder(document.uri) : undefined;
  const candidates = [];
  if (folder) candidates.push(path.join(folder.uri.fsPath, 'build', 'kyma'));
  if (document) {
    let directory = path.dirname(document.fileName);
    for (;;) {
      candidates.push(path.join(directory, 'build', 'kyma'));
      const parent = path.dirname(directory);
      if (parent === directory) break;
      directory = parent;
    }
  }
  return candidates.find(candidate => fs.existsSync(candidate)) || 'kyma';
}
...
async function runActive(command) {
  ...
  if (!editor || editor.document.languageId !== 'kyma') return;
  await editor.document.save();
  const terminal = vscode.window.createTerminal({
    name: command === 'run' ? 'Kyma Run' : 'Kyma Check',
...
function toDiagnostic(entry, document) {
  ...
  diagnostic.source = 'kyma';
  return diagnostic;
}

function validate(document, collection) {
  if (document.languageId !== 'kyma') return;
  ...
  process.on('error', error => {
    ...
    if (!missingExecutableReported) {
      ...
      vscode.window.showWarningMessage(`Kyma diagnostics could not start: ${error.message}. Set kyma.executable.`);
    }
  });
  process.on('close', () => {
    if (validationProcesses.get(key) !== process) return;
...
async function importPathCompletions(document, position) {
  ...
  const files = await vscode.workspace.findFiles('**/*.ky', '**/{node_modules,build,build-*}/**', 300);
...
async function importedMemberCompletions(document, position) {
  ...
  if (namespaceMembers[receiver])
    return namespaceMembers[receiver].map(name => completionItem([
      name, vscode.CompletionItemKind.Method, `Kyma ${receiver} operation`
    ]));
...
function activate(context) {
  const diagnostics = vscode.languages.createDiagnosticCollection('kyma');
  const runButton = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 100);
  runButton.text = '$(play) Run Kyma';
  runButton.command = 'kyma.runFile';
  runButton.tooltip = 'Run the active Kyma file';

  const updateButton = editor => {
    if (editor && editor.document.languageId === 'kyma') runButton.show();
    else runButton.hide();
  };
  ...
  context.subscriptions.push(
    diagnostics,
    runButton,
    vscode.commands.registerCommand('kyma.runFile', () => runActive('run')),
    vscode.commands.registerCommand('kyma.checkFile', () => runActive('check')),
